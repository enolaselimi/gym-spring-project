...
public class ClientRepositoryImpl implements ClientRepository {
    ...
    @Override
    public List<Exercise> findAllExercises(Integer clientId,Filter nameFilter) {
        ...
        if(nameFilter.getValue() != null){
            FIND_ALL_EXERCISES += "AND e."+nameFilter.getField()+" "+nameFilter.getOperator()+" '%"+nameFilter.getValue()+"%' ";
        }

        if(nameFilter.getSort() != null) {
            FIND_ALL_EXERCISES += "ORDER BY e." + nameFilter.getField() + " " + nameFilter.getSort();
        }
        if(nameFilter.getPageSize() != null && nameFilter.getPageNumber() != null){
            return entityManager.createQuery(FIND_ALL_EXERCISES, Exercise.class)
                    .setFirstResult((nameFilter.getPageNumber() - 1) * nameFilter.getPageSize())
                    .setMaxResults(nameFilter.getPageSize())
                    .setParameter("clientId", clientId)
                    .getResultList();
        }
        return entityManager.createQuery(FIND_ALL_EXERCISES, Exercise.class)
